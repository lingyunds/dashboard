...
class TestCryptoCurrencyModel:
    ...
    def test_symbol_max_length(self):
        ...
        crypto = CryptoCurrency.objects.create(
            name="Test Coin", symbol="TEST12345"  # 10个字符，应该符合 max_length=10
        )
...
class TestPriceHistoryModel:
    ...
    def test_price_history_ordering(self, crypto):
        """测试价格历史记录的排序"""
        # 创建多个价格记录
        PriceHistory.objects.create(cryptocurrency=crypto, price=Decimal("40000.00"))
        time.sleep(0.1)  # 内存数据库操作保证不了先后顺序
        PriceHistory.objects.create(cryptocurrency=crypto, price=Decimal("45000.00"))
        time.sleep(0.1)
        PriceHistory.objects.create(cryptocurrency=crypto, price=Decimal("42000.00"))
